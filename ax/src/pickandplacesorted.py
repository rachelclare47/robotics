...
def main():
 ...
 # Move to the desired starting angles
    ...
    for i in range(0,4):
        block = "block"+str(i)
        try: 
            tranformation = tfBuffer.lookup_transform('base', block, rospy.Time(), rospy.Duration(1.0))
            trans = transformation.transform.translation
            x = trans.x
            y = trans.y
            z = trans.z
            block_pose = (Pose(position=Point(x,y,z), orientation=overhead_orientation))
            print("\nPicking...")
            pnp.pick(block_pose)
            block_pose_place = (Pose(position=Point(x=-0.2+extra, y=1, z=0.75), orientation=overhead_orientation))
            print("\nPlacing...")
